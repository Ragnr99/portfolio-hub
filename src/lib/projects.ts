...
export const PROJECTS: Project[] = [
  ...
  },
  {
    ...
    longDescription:
      'Five tools over the game\'s own data. The Palpedia covers all 288 Pals with combat stats, work suitability, partner skills, drops and type matchups. The breeder works both directions: two parents to a child, or a target Pal to every parent pair that makes it. It ships the game\'s entire 44,850-pair breeding table. The widely repeated "average the two CombiRanks" rule reproduces about 69% of real results. The Passive Dex catalogues all 115 passive skills in 1.0 and stacks any four of them into one set of totals, with every number parsed out of the game\'s own descriptions by a build script that halts on any line it cannot match. The Quest Tree carries all 117 quests from DT_PalQuestData with their objectives in the order the game asks for them: one main chain about 30 quests long, and 59 side missions shelved by questgiver and cross-linked by map distance. The map is a Leaflet build of both worlds: the Palpagos Islands with 13,463 markers across 14 toggleable layers, and the World Tree with its own 379 across 13, each on its own coordinate transform reverse-engineered from the game files. It carries per-Pal day/night spawn clouds and all 64 readable journal notes. There is also a desktop overlay that pins the element chart over the game and hides itself whenever a menu is open.',
